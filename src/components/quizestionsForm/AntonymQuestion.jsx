...
function AntonymQuestion({
  ...
  // دالة بتشغّل الصوت لأي كلمة
  const speak = (word) => {
    if (!("speechSynthesis" in window)) return;
    const utterance = new SpeechSynthesisUtterance(word);
    utterance.lang = "de-DE";
    // جرّب تدور على صوت ألماني متاح
  ...
  // ينطق الكلمة لما يختارها المستخدم
  ...
  // أول ما يحصل تحقق ينطق الإجابة الصح مرة واحدة
  useEffect(() => {
    if (nextQuiz && !spokenAfterCheck) {
      ...
      // رجّع الفلاج لو ابتدى سؤال جديد
      setSpokenAfterCheck(false);
    }
